...
if not SECRET_KEY:
    ...
    # Set NOVA_EARN_SECRET in production.
...
def verify_password(stored_password, supplied_password):
    """
    Supports the old SHA-256 hashes so existing users do not get locked out.

    When an old SHA-256 password is successfully verified, the caller can
    replace it with the stronger scrypt hash.
    """
    ...
    old_hash = legacy_hash(supplied_password)
...
def protect_state_changing_requests():
    ...
    # These are the only POST routes in this application.
    # The withdrawal form and administrative operations are protected.
    protected_endpoints = {
        "register",
        "login",
        "invest",
...
def init_db():
    ...
    # ---------------------------------------------------------------
    # Safe schema upgrades for existing NovaEarn databases.
    # ---------------------------------------------------------------
    ...
    # ---------------------------------------------------------------
    # Existing users: assign referral codes.
    # ---------------------------------------------------------------
    ...
    for position, row in enumerate(existing, start=1):
        if not row["name"]:
            ...
            clean_name = "".join(
                ch
                for ch in row["name"].upper()
                if ch.isalnum()
            )[:12] or "USER"

        code = f"{clean_name}{position:03d}"
    ...
    # ---------------------------------------------------------------
    # Upgrade the existing known admin account to admin role.
    # ---------------------------------------------------------------
...
def login():
    if request.method == "POST":
        ...
        identifier = (
            f"{request.remote_addr or 'unknown'}:{e}"
        )
        ...
        # Upgrade old SHA-256 passwords automatically.
        if legacy:
            c.execute(
                """
                UPDATE users
...
def withdraw():
    ...
    if request.method == "POST":
        ...
        if destination == "Bank account":
            ...
            # Bank code is irrelevant for wallet destinations.
            bank_code = ""
...
def admin_update_withdrawal(
    ...
    try:
        ...
        if action == "reject":
            # Refund the amount because it was reserved
...
if __name__ == "__main__":
    # Debug must NOT be enabled on a production server.
    debug_mode = (
        os.environ.get(
            "NOVA_DEBUG",
            "0",
